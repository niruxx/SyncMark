...
router.get('/passwords', (req, res) => {
  ...
  res.json(statements.listPasswordsBySort[sort].all({ q, qLike, favorite }));
});

router.get('/passwords/export', (req, res) => {
  const all = statements.listPasswordsBySort[DEFAULT_SORT]
    .all({ q: '', qLike: '', favorite: 0 })
    .map((row) => statements.getPassword.get(row.id));
...
router.get('/passwords/:id', (req, res) => {
  const row = statements.getPassword.get(req.params.id);
...
router.post('/passwords', (req, res) => {
  const fields = fieldsFromBody(req.body, null);
...
router.put('/passwords/:id/favorite', (req, res) => {
  ...
  statements.setPasswordFavorite.run({ id: existing.id, favorite: req.body.favorite ? 1 : 0 });
  const row = statements.getPassword.get(existing.id);
  res.json({ ...row, password_enc: undefined });
});

router.delete('/passwords/all', (req, res) => {
  statements.deleteAllPasswords.run();
  res.status(204).end();
});
...
router.delete('/passwords/:id', (req, res) => {
  ...
  statements.deletePassword.run(existing.id);
  res.status(204).end();
});
